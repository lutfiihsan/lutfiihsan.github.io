...
async function fetchPortfolioData() {
    const apiUrl = window.__PORTO_API_URL__ || import.meta.env.VITE_API_URL || '';
    if (apiUrl) {
        try {
            const res = await fetch(`${apiUrl.replace(/\/$/, '')}/api/portfolio`);
            if (res.ok) {
...
async function fetchDataAndRender() {
    try {
        ...
        const isProjectPage = path === '/project' || path.endsWith('/project');
...
function renderProjectDetail(projects) {
    ...
    if (!project) {
        container.innerHTML = `
            <div style="text-align:center; padding: 10rem 2rem;">
                ...
                <a href="/#work" class="btn">Go Back to Projects</a>
            </div>`;
        return;
    }
    ...
    const html = `
        <div class="detail-nav-top">
            <a href="/#work" class="btn btn-outline back-btn-top"><i class="fas fa-chevron-left"></i> Back to Portfolio</a>
        </div>
            ...
            <div class="split right-split">
                ...
                <div class="detail-actions">
                    ...
                    <div style="margin-top: 3rem;">
                        <a href="/#work" class="back-link"><i class="fas fa-arrow-left"></i> Back to Portfolio</a>
                    </div>
                </div>
            </div>
        </div>
